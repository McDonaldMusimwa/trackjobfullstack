  ...
  return (
    <header className="w-40 p-2 h-full fixed inset-y-0 left-0">
      <nav className="flex flex-col justify-center mt-2 ml-2">
        <a href='/'><h1 className="m-2 text-black text-2xl font-bold">Trekjob</h1></a>
        {navlinks.map((link) => {
          return (
            <Link
              to={link.link}
        ...
        <button className='w-32 flex items-center gap-3 p-3 rounded-lg bg-lime-5 hover:bg-gray-800 hover:text-white transition-colors mb-2'>+ Add Application</button>
      </nav>
    </header>
  )
}
